...
def get_appsite_mysql():
    ...
    if r.llen('apiclassid') == 0:
        print r.keys('*')
        #获得表中有多少条数据
        result = cur.execute("select api_class_id,doc_website from api_class group by name")
...
def get_app_introduction():
    ...
    downloadIndex = 1
    donext = 1
    classidlen = r.llen('failapiid')
    localclassidlist = []
    localclassversionlist = []
    while(r.llen('failapiid') != 0):
        try:
            localclassidlist = []
            localclassversionlist = []
            url = r.lpop('failapilink')
            id = r.lpop('failapiid')
            if url != '' :
                ...
                if return_1 == -1:
                    r.lpush('againfailapilink',url)
                    r.lpush('againfailapiid',id)
                    return -1
                else:
                    for list in lists:
...
if __name__ == '__main__':
    #download('https://developer.android.com/reference/android/app/ActivityManager.ProcessErrorStateInfo.html',1)
    #get_appsite_mysql()
