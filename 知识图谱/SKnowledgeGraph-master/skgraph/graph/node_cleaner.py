...
class NodeCleaner:
    ...
    def get_clean_node_name(self, node):
        if "labels_en" in node.keys():
            return node["labels_en"]
        if "api_qualifier_name" in node.keys() and node["api_qualifier_name"]:
            return node["api_qualifier_name"]

        if "full type method name" in node.keys() and node["full type method name"]:
            return node["full type method name"]

        if "name" not in node.keys():
            return ""

        node_base_name = node["name"]
        if node.has_label("java method parameter"):
            return "Method Parameter: " + node_base_name

        if node.has_label("java method return value"):
            return "Method Return Value: " + node_base_name

        if node.has_label("java throw exception description"):
            return "Throw Exception Directive: " + node_base_name

        if node.has_label("java class") and "." not in node_base_name:
            return "Class: " + node_base_name

        if node.has_label("java method") and "." not in node_base_name:
            return "Method: " + node_base_name

        if node.has_label("java constructor") and "." not in node_base_name:
            return "Constructor: " + node_base_name

        if node.has_label("java field") and "." not in node_base_name:
            return "Field: " + node_base_name

        if self.is_merged_wikipedia_node_to_constraint_node(node):
            # todo remove this by doing this clean background
            team = self.clean_for_merged_wikipedia_node_to_constraint_node(node)
            return team["name"]

        return self.remove_emoji(node_base_name)
    ...
    def clean_node_properties(self, node):
        ...
        if self.is_merged_wikipedia_node_to_constraint_node(node):
            return self.clean_for_merged_wikipedia_node_to_constraint_node(node)
    ...
    def is_merged_wikipedia_node_to_constraint_node(self, node):
        keys = node.keys()
        if "site:enwiki" in keys and "predicate" in keys and "subject" in keys:
            return True
        return False
...
class NodeRelationFilter:
    ...
    def check_relation_validity(self, relation):
        if relation is None:
            return False
        if relation.type() == "has constraint":
            return False
